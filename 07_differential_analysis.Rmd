...
```{r load_data_fil_functional, comment="", message=FALSE, warning=FALSE}
load("data/data_host_filtered.Rdata")
```

```{r load_data_capwild, comment="", message=FALSE, warning=FALSE}
...
## Captive vs Wild
### Structural zeros
```{r struct_zero_capwild, comment="", message=FALSE, warning=FALSE}
...
#### MAG level
```{r ancom_rand_capwild, comment="", message=FALSE, warning=FALSE, eval=FALSE}
...
```{r ancombc_rand_plot_capwild, comment="", message=FALSE, warning=FALSE, fig.height=20, fig.width=8, fig.fullwidth=TRUE}
...
```
```{r ancom_rand_volcano_sp, comment="", message=FALSE, warning=FALSE}
...
#### Genus level
```{r ancom_rand_genus_capwild, comment="", message=FALSE, warning=FALSE, eval=FALSE}
...
```{r ancom_rand_genus1_capwild, comment="", message=FALSE, warning=FALSE}
...
```{r ancombc_rand_genus_plot_capwild, comment="", message=FALSE, warning=FALSE, fig.height=12, fig.width=8, fig.fullwidth=TRUE}
...
```{r ancom_rand_volcano_genus_capwild1, comment="", message=FALSE, warning=FALSE}
...
```{r ancom_phylum_capwild, comment="", message=FALSE, warning=FALSE, eval=FALSE}
...
```{r ancom_rand_phylum_capwild, comment="", message=FALSE, warning=FALSE}
...
```{r ancom_table_genus_capwild, comment="", message=FALSE, warning=FALSE}
...
```{r ancom_save_wildcapt, comment="", message=FALSE, warning=FALSE, eval=FALSE}
...
```{r spiro_capwild, comment="", message=FALSE, warning=FALSE}
...
```{r zero_phylo2, comment="", message=FALSE, warning=FALSE}
phylo_samples <- sample_metadata %>%
  column_to_rownames("sample") %>%
  sample_data() 
phylo_genome <- genome_counts_filt %>%
...
```{r arachea_abundances, comment="", message=FALSE, warning=FALSE}
...
#### MAG level
```{r ancom_rand_pre_post, comment="", message=FALSE, warning=FALSE, eval=FALSE}
set.seed(1234)
...
```{r ancom_rand1_pre_post, comment="", message=FALSE, warning=FALSE}
taxonomy <- data.frame(physeq_sample@tax_table) %>%
  rownames_to_column(., "taxon") %>%
  mutate_at(vars(phylum, order, family, genus, species), ~ str_replace(., "[dpcofgs]__", ""))
...
```{r ancombc_rand_plot_pre_post, comment="", message=FALSE, warning=FALSE, fig.height=5, fig.width=6, fig.fullwidth=TRUE}
ancombc_rand_table_mag%>%
  mutate(genome=factor(genome,levels=ancombc_rand_table_mag$genome)) %>%
...
```{r ancom_rand_volcano_pre_post, comment="", message=FALSE, warning=FALSE}
