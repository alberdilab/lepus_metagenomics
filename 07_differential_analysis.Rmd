...
```{r ancombc_rand_plot_phy_capwild, comment="", echo=FALSE, message=FALSE, warning=FALSE, fig.height=10, fig.width=8, fig.fullwidth=TRUE, eval=FALSE}
ancombc_rand_table_mag %>%
  mutate(genome=factor(genome,levels=ancombc_rand_table_mag$genome)) %>% 
ggplot(., aes(x=lfc_dietWild, y=forcats::fct_reorder(species, lfc_dietWild), fill=phylum)) + #forcats::fct_rev()
  geom_col() + 
...
```{r ancombc_rand_plot_phy_pre_post, comment="", echo=FALSE, message=FALSE, warning=FALSE, fig.height=5, fig.width=8}
ancombc_rand_table_mag %>%
  mutate(genome=factor(genome,levels=ancombc_rand_table_mag$genome)) %>% 
ggplot(., aes(x=lfc_dietPre_grass, y=forcats::fct_reorder(species, lfc_dietPre_grass), fill=phylum)) + #forcats::fct_rev()
  geom_col() + 
  scale_fill_manual(values=tax_color) + 
  geom_hline(yintercept=0) + 
#  coord_flip()+
...
## wild vs post grass captive

### Structural zeros

```{r grass_struct_zero, comment="", echo=FALSE, message=FALSE, warning=FALSE}
wild_samples <- sample_metadata %>% 
  filter(diet == "Wild") %>%
  dplyr::select(sample) %>%
  pull()
...
structural_zeros <- genome_counts %>% 
   rowwise() %>% #compute for each row (genome)
   mutate(all_zeros_wild = all(c_across(all_of(wild_samples)) == 0)) %>% # set true if all samples in TJ1 have zeros
   mutate(all_zeros_post = all(c_across(all_of(post_samples)) == 0)) %>% # set true if all samples in TJ2 have zeros
   mutate(average_wild = mean(c_across(all_of(wild_samples)), na.rm = TRUE)) %>% # get average genome counts across TJ1
   mutate(average_post = mean(c_across(all_of(post_samples)), na.rm = TRUE)) %>% # get average genome counts across TJ2
   filter(all_zeros_wild == TRUE || all_zeros_post==TRUE)  %>% # filter only genomes with structural zeros
   mutate(present = case_when(
      all_zeros_wild & !all_zeros_post ~ "post",
      !all_zeros_wild & all_zeros_post ~ "wild",
      !all_zeros_wild & !all_zeros_post ~ "None",
      TRUE ~ NA_character_
    )) %>%
   mutate(average = ifelse(present == "wild", average_wild, average_post)) %>%
   dplyr::select(genome, present, average) %>%
   left_join(genome_metadata, by=join_by(genome==genome)) %>%
   arrange(present,-average)
```
...
```{r grass_zero_phylo2, comment="", echo=FALSE, message=FALSE, warning=FALSE}
#phyloseq object considering structual zeros
phylo_samples <- sample_metadata %>% 
                    column_to_rownames("sample") %>% 
                    sample_data() #convert to phyloseq sample_data object
...
```{r grass_filter_pre_post, comment="", echo=FALSE, message=FALSE, warning=FALSE}
physeq_sample <- subset_samples(physeq_genome_filtered, diet %in% c("Wild","Post_grass"))
physeq_sample <- prune_taxa(taxa_sums(physeq_sample)>0, physeq_sample)
```


#### MAG level
```{r grass_ancom_rand_pre_post, comment="", echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
set.seed(1234) #set seed for reproducibility
ancom_rand_output_mag_pre_post = ancombc2(data = physeq_sample, 
                  assay_name = "counts", 
                  tax_level = NULL, #change to agglomerate analysis to a higher taxonomic range
...
```{r grass_ancom_rand1_pre_post, comment="", echo=FALSE, message=FALSE, warning=FALSE}
taxonomy <- data.frame(physeq_sample@tax_table) %>%
  rownames_to_column(., "taxon") %>%
  mutate_at(vars(order, family, genus, species), ~ str_replace(., "[dpcofgs]__", ""))%>%
  mutate(across(c(family,genus, species), na_if, ""))%>%
