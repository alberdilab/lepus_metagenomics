...
sample_metadata <- read_tsv("data/DMB0165_metadata.tsv") %>%
    rename(sample=1)
```
...
read_counts <- read_tsv("data/DMB0165_counts.tsv.gz") %>%
    rename(genome=1) %>% 
...
genome_metadata <- read_tsv("data/DMB0165_mag_info.tsv.gz") %>%
...
genome_coverage <- read_tsv("data/DMB0165_coverage.tsv.gz") %>%
    rename(genome=1) %>% 
    select(one_of(c("genome",sample_metadata$sample)))%>% 
...
genome_tree <- read_tree("data/DMB0165.tree.gz")
